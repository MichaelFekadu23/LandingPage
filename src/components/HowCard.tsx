export default function HowCard({
  ...
  return (
    <article className="relative flex h-full flex-col overflow-hidden rounded-[24px] bg-white p-5 shadow-[0_8px_40px_rgba(0,0,0,0.06)] sm:rounded-[28px] sm:p-6 lg:rounded-[30px] lg:p-8">
      ...
      {/* STEP BADGE */}
      <div
        className="mt-5 inline-flex w-max items-center rounded-[12px] bg-[#2F2F2F] px-3 py-1.5 text-xs text-white shadow-[0_3px_10px_1px_#01C705] sm:mt-6 sm:rounded-[14px] sm:px-4 sm:py-2 sm:text-sm lg:rounded-[15px]"
      >
        Step {step}
      </div>

      {/* TEXT */}
      <h4 className="mt-5 text-lg font-inter font-semibold text-[rgba(26,26,26,0.9)] sm:mt-6 sm:text-xl lg:mt-7 lg:text-2xl">
        {title}
      </h4>
        ...
        {bullets.map((b, i) => (
          <li key={i} className="flex items-start gap-2">
            <span className="relative top-[8px] sm:top-[9px]">
              <span className="block h-[5px] w-[5px] rounded-full bg-[rgba(26,26,26,0.65)] sm:h-[6px] sm:w-[6px]" />
            </span>
            <span className="leading-relaxed font-inter text-[14px] font-[500] text-[rgba(26,26,26,0.65)]">{b}</span>
          </li>
        ))}
      </ul>
    </article>
